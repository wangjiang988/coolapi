...
class SystemControl extends BaseControl{
	/**
	 * 管理员资料 name id group
	 */
...
//			$state = $state ? L('nc_succ') : L('nc_fail');
	...
	{
	    $json          =  getSession('loginUser');
	    $loginUser     =  json_decode($json,true);
	    if(!$loginUser)  ajaxReturn(L('E400'),400);
	...
	{
	      $arr       =  json_decode(decrypt($token,C('encrypt_key')),true);
	      $realToken =  getSession('token'.$arr['time']);
	     
	      if($realToken  == $token)
	      {
	          setSession('token'.$arr['time'], null);
	          return true;
	      }
	      else{
// 	          setSession('token'.$arr['time'], null);
	          ajaxReturn(L('E601'),601);
	      }
	}
	...
	public function  getCsrfOp()
	{
	    ...
	    $csrf    =  encrypt(json_encode($data),C('encrypt_key'));
	    setSession('token'.$now, $csrf);
	    ajaxReturn('获取成功','200',array('token' =>$csrf));
	}
