...
class Db{
	...
	public static function getLastId($host = 'master'){
		...
		if (!$id){
			$id = mysql_fetch_array(self::query('SELECT last_insert_id() as id',$host),MYSQL_ASSOC);
			$id = $id['id'];
		}
		return $id;
	}
	...
	public static function getCount($table, $condition = null, $host = 'slave'){
		...
		$result = mysql_fetch_array(self::query($sql,$host),MYSQL_ASSOC);
		return $result['count'];
	}
	...
	public static function showColumns($table, $host = 'slave'){
		self::connect($host);
		$sql = 'SHOW COLUMNS FROM `'.$GLOBALS['config']['tablepre'].$table.'`';
		$result = self::query($sql,$host);
		$array = array();
		while ($tmp=mysql_fetch_array($result,MYSQL_ASSOC)){
