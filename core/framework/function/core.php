...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			echo '<dd><p /><p /><p /><p /> '.$lang['company_name'].'</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit;
...
function write_file($filepath, $data, $mode = null){
	...
	if ($data === '') $data = '\'\'';
	$data = "<?php defined('CoolAPI') or exit('Access Invalid!'); return ".$data."\n?>";
...
function throw_exception($error){
	if (!defined('IGNORE_EXCEPTION')){
		ajaxReturn($error,'500','exception');
	}else{
		exit();
	}
}
 ...
 * @return bool 返回值
 */
function wcache($key = null, $data = array(), $prefix = '',  $ttl = null,  $serialize = true){
	if (empty($key) || C('cache.type') == 'file') return false;
	$obj_cache = Cache::getInstance(C('cache.type'));
...
function getChat($layout){
	if (!file_exists(BASE_CORE_PATH.'/framework/libraries/chat.php')) return '';
 ...
 * @param string $type
 */
function ajaxReturn($msg='',$code=1,$data='',$page=null,$category = null) {
    $result               =  array();
    $result['code']       =  $code.'';
    $result['msg']        =  $msg;
    ...
    $result['data']       =  $data;
    $result['loginUser']  =  json_decode(getSession('loginUser'));  
    if($category != null){
...
function setRoute($route){
    if(empty($route['act']) || empty($route['op']))
        ajaxReturn("This route not exist!",'101');
    $_GET['act'] = $route['act'];
    $_GET['op']  = $route['op'];
    return 1;
}
...
function SS($key,$value = null )
{
    if(!empty($value))
    {
        ...
        $where      = array('k' => $update['k']);
        $session  = Model('session')->where($where)->find();  
        if ($session){
            $ret  = Model('session')->where(array('k' => $update['k']))->update($update);
        }else{
            $ret  = Model('session')->insert($update);
        }
