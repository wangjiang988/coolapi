...
class Model{
	...
	{
	    return $this->field($field)->order(' '.$this->get_pk().' asc')->select();
	}
    ...
    protected function parse_options($options=array()) {
        ...
        if ($options['where'] === true){
        	//更新或删除全部，此处不需作处理
        }elseif(is_array($options['where']) && C('check_field')) {
        	//字段检查
            ...
            foreach ($options['where'] as $key=>$val){
            	if ($this->table_name == '') break;
            	if (strpos($key,'|') || strpos($key,'&') || in_array($key,array('_op'))) break;	//除去 name|title&nickname和自定义运算符的情况
            	if (!in_array($key,$this->fields[$this->table_name],true) || (!is_scalar($val) && !is_array($val))){
...
//            if(isset($pkValue)) $data[$pk]   =  $pkValue;
    ...
    public function update($data='',$options=array()) {
        ...
        // 数据处理
        if (C('check_field')) $data = $this->chk_field($data);
        // 分析表达式
        $options =  $this->parse_options($options);
...
//                $data[$this->get_pk()]  = $insertId;
    ...
    public function get_pk() {
    	if (C('check_field')){
    		return isset($this->fields[$this->table_name]['_pk'])?$this->fields[$this->table_name]['_pk']:$this->pk;
    	}else{
    ...
    */
    ...
    public function getPager($style = null)
    {
        return pagecmd('obj',$style);
    }
	/**
	 * 获取分页总数
	 *
...
//         d($sql);die;  //输出查询sql TODOwangjiang
        return $sql;
    }
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                    ...
                    $data = is_string($val[1])? explode(',',$val[1]):$val[1];
                    if($data[0] && $data[1]) {
                        $whereStr .=  ' ('.$key.' BETWEEN '.$this->parseValue($data[0]).' AND '.$this->parseValue($data[1] + 86400).' )';
                    } elseif ($data[0]) {
                        $whereStr .= $key.' '.$this->comparison['gt'].' '.$this->parseValue($data[0]);
                    } elseif ($data[1]) {
	...
	public function lockAttr($options){
		if (isset($options['attr'])){echo $options['attr'];exit;
