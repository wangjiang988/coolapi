...
define('WX_PAY_API_PATH', BASE_DATA_PATH.'/api/wxpayapi');
// define('BASE_UPLOAD_PATH',BASE_DATA_PATH.'/upload');
define('BASE_UPLOAD_PATH', (dirname(BASE_PATH)).'/xiagao/Public/upload/');
...
//在这里截取路由
$routeMap =  require(BASE_PATH.'/config/routeMap.ini.php');
$now_route = 'index';  //当前默认请求路由
if(!$config['is_root']){
    $now_route = trim($_GET['json']);        
}else {
    ...
    if ($uri  != '/') {
        $uriArr = explode('/',$uri);
        array_shift($uriArr);
        $now_route = isset($uriArr[0]) ? $uriArr[0] : 'index';
    }
    
    if(!key_exists($now_route, $routeMap))
    {
        ajaxReturn('route error!','102');
    }
}

setRoute($routeMap[$now_route]);
