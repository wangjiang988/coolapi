	...
	public function advOp(){
		...
		if (chksubmit()){
			if (is_array($_POST['del_id']) && !empty($_POST['del_id'])){
			    ...
			    $ap_ids = Model()->table('adv')->where(array('adv_id'=>array('in',$_POST['del_id'])))->field('ap_id')->getfield(1);
			    Db::delete("adv",$where);
			    if (!empty($ap_ids)){
			    	$ap_ids = array_unique($ap_ids);
			    	foreach ($ap_ids as $v) {
			    		$adv->makeApCache($v);
			    	}
			    }
